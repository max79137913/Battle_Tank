...
public class BattleCity extends Applet implements Runnable, KeyListener, MouseListener {
	...
	public void paint(Graphics g)
	{
		...
		showStatus("Game Level: " + new Integer(gameLevel).toString() + "  Tanks Left: " + new Integer(tanksLeft).toString()+ "    ShieldsLeft:    "
				+ new Integer(shieldsLeft).toString());
	}
	...
	public void alldie() {
		for(int i=1;i<=9;i++){
		tank[i].exploded = true;   tanksLeft--;
		}
		gameOver=true;
	}
	public void cheat(){
	...
	void simulateTanks(){
		for(int i=0;i<MAX;i++){
			if(tank[i].isAlive){
			//	handleIntersections(); 為了chase效果更明顯先//
				simulateMovement(i);
				if(i!=0) simulateFire(i);
				if(tank[i].isswitch==true)switchcpudirection();
				
				
				if(tank[i].isswitch!=true)chase(i);
			}
			cooperate(i);
		}
	}
	...
	void cooperate(int i)
	{
		...
		if(i==1||i==4||i==7)
		{
			...
			distance=(int)Math.sqrt((tank[i].x-tank[i+1].x)*(tank[i].x-tank[i+1].x) + (tank[i].y-tank[i+1].y)*(tank[i].y-tank[i+1].y));
			distance2=(int)Math.sqrt((tank[i].x-tank[i+2].x)*(tank[i].x-tank[i+2].x) + (tank[i].y-tank[i+2].y)*(tank[i].y-tank[i+2].y));
			distance3=(int)Math.sqrt((tank[i+1].x-tank[i+2].x)*(tank[i+1].x-tank[i+2].x) + (tank[i+1].y-tank[i+2].y)*(tank[i+1].y-tank[i+2].y));
			
			if (distance<50) {tank[i].isswitch=false;tank[i+1].isswitch=false;System.out.print("chase!! ");}
			if (distance2<50) {tank[i].isswitch=false;tank[i+2].isswitch=false;System.out.print("chase!! ");}
			if (distance3<50) {tank[i+1].isswitch=false;tank[i+2].isswitch=false;System.out.print("chase!! ");}
	...
	void switchcpudirection()
	{
		
		int sum;
		int way=(int)(((Math.random()*10)%4)+1);
		
		for(int i=1;i<10;i++)
		{
			sum=(((int)(Math.random()*10))+i)%10;
			if(sum==1)
			{
				tank[i].turn(way);
	...
	void chase(int i)
	{
		...
		else if(tank[i].x < tank[0].x){
			tank[i].turn(2);
		}	
		else if(tank[i].y > tank[0].y){
			tank[i].turn(1);
		}						
		else if(tank[i].y < tank[0].y){
			tank[i].turn(3);
